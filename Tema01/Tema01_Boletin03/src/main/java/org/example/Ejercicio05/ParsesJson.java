...
import org.example.Ejercicio08.Pelicula;
import org.example.Ejercicio08.Puntuacion;
...
public class ParsesJson {
    ...
    public static Pelicula[] parsearPeliculas() throws IOException {
        Pelicula[] peliculas = null;
        StringBuilder sb = new StringBuilder();
        try (InputStream is = ParsesJson.class.getResourceAsStream("/Datasets/peliculas.json");
             BufferedReader br = new BufferedReader(new InputStreamReader(is,
                     StandardCharsets.UTF_8))) {
            String line;
            while ((line = br.readLine()) != null) {
            ...
            JSONObject jsonObject = new JSONObject(tokener);
            // Peliculas
            peliculas = new Pelicula[jsonObject.length()];
            JSONObject peliculasObject = jsonObject.getJSONObject("peliculas");
            JSONArray peliculasArray = peliculasObject.getJSONArray("peliculas");
            for (int i = 0; i < peliculasArray.length(); i++) {
                JSONObject peliculaObject = peliculasArray.getJSONObject(i);
                // Id, titulo, director, estreno, duracion
                ...
                for (int j = 0; j < generosArray.length(); j++) {
                    generos[j] = generosArray.getString(j);
                }
                // Puntuaciones
                JSONObject puntuacionesObject = peliculasObject.getJSONObject("puntuaciones");
                double imdb = puntuacionesObject.getDouble("imdb");
                int rt = puntuacionesObject.getInt("rt");
                Puntuacion puntuacion = new Puntuacion(imdb, rt);
                // Añado la pelicula a la LISTA
                peliculas[i] = new Pelicula(id, titulo, director, estreno, duracionMin, generos, puntuacion);
            }
            return peliculas;
        }
    }
